...
## 0.6.1
- **UI 更明顯的蘋果風格改版**：改用 Apple 系統灰階（淺色 #f5f5f7／深色純黑）、放大標題（large title）、主要按鈕改藥丸狀、頂部改毛玻璃（frosted）半透明列。整體更接近原生 App。
- **修正深色模式筆記框變白底**：textarea 沒有套用深色，現在已修。

## 0.6.0
- **UI/UX 優化（統一設計基礎，朝原生 App 風格）**：精修設計系統 — 圓角、陰影層次、字級階層、按鈕觸感（緩動曲線 + 按壓回饋），全頁面一致提升。
- **修正 Live 頁閒置時的黑色橫條**：還沒有字幕前不再顯示突兀的黑色字幕區。
- **快速錄音改成先開選項面板**：點「快速錄音」會展開面板，可選來源（麥克風／系統音／兩者）、即時模型、純錄音，再開始；支援雙軌擷取。修掉原本只能麥克風、無法調整選項的問題。

## 0.5.5
- **修正截圖**：改由瀏覽器擷取畫面再上傳 — macOS 螢幕錄製權限是綁定 App 的，背景伺服器無法用 `screencapture`。現在點截圖會跳出瀏覽器的「分享畫面」選擇（用瀏覽器既有權限，免額外授權），擷取後附加到會議。

## 0.5.4
- **新增截圖功能**：會議頁可一鍵擷取整個螢幕（如投影片）附加到該會議，縮圖檢視、可刪除。

## 0.5.3
- **設定頁新增「資料管理」**：顯示總用量與分類大小（音檔 data/、資料庫、語音模型、MLX 模型），並列出最佔空間的會議（音檔），點進去可刪除釋放空間。

## 0.5.2
- **任意頁面快速錄音**：除了錄音頁，其他頁面右下角會有「● 快速錄音」浮動鈕，直接開始麥克風錄音（建立新 session、即時辨識），錄音中顯示計時與「■ 停止」。需要選系統音／模型／多人分群再進錄音頁。

## 0.5.1
- **全頁面錄音指示**：錄音進行中時，任何頁面的頂端都會顯示紅色「● 錄音中」，點一下回到錄音頁 — 不會再忘記正在錄而切走。

## 0.5.0
- **新增「純錄音」模式（省電·不即時辨識）**：電腦快燒起來、或只想快速把會議錄下來時，勾「🪫 純錄音」只存音檔、**零推論、不發熱**；錄音中可隨時切換（免停止重來），事後再「重新語音辨識」即可。這是降溫最直接的解 — 擷取階段完全不跑模型。
- **降噪開關移到錄音頁**：可直接在 live 頁開關「降噪」（事後重新辨識時生效），不用再進設定。
- **錄音中調整不需停止重來**：即時模型、純錄音、降噪都可在錄音進行時切換。

## 0.4.9
- **「可能是同一人」加上「不是同一人」**：可以否決誤判的配對，被否決的組合不再出現在合併建議裡。

## 0.4.8
- **標籤與說話者輸入自動補全**：加標籤時會提示過去用過的標籤；在回顧頁點說話者改名時，改用內嵌輸入框＋已知語者清單（直接挑，不用每次重打名字）。

## 0.4.7
- **修正語者頁出現 0 筆紀錄的幽靈語者**：沒有任何逐字稿引用的聲紋（常見於重新辨識後被孤立的暫名 我68、我70…）會顯示成 0 場 0 句，現在直接隱藏 — 沒有紀錄就不該進語者頁。

## 0.4.6
- **修正「可能是同一人」會把沒有語音的語者也拿來比較**：沒有可試聽片段（無 >800ms 發言）的聲紋無法用耳朵確認，現在不再出現在合併建議裡 — 清掉一堆無意義、無法驗證的配對。

## 0.4.5
- **新增現場筆記 → 摘要參考來源**：錄音頁與會議頁可邊開會邊記重點（自動儲存）。產生摘要時，筆記會當成**可信來源**餵給模型（優先採用其中的人名、日期、決議），且筆記裡的人名／日期不會被防瞎掰機制誤刪。讓摘要能補上逐字稿沒明講、但你知道的事實。

## 0.4.4
- **新增「嘈雜環境降噪」實驗選項**(DeepFilterNet3·ANE，設定 → 🧪 實驗性功能)：辨識前先去背景噪音，於上傳辨識與「重新語音辨識」生效。**預設關閉** — 實測乾淨音訊反而會多出辨識錯誤、且增加辨識耗時，僅真正嘈雜的錄音才有幫助。降噪跑在 Neural Engine、不占 GPU。
